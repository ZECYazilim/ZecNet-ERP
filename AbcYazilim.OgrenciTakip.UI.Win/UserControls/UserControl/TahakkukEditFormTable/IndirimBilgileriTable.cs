    ...
    public partial class IndirimBilgileriTable : BaseTablo
    {
        ...
        protected override void HareketEkle()
        {
            bool HizmetAlindi(long hizmetId)
            {
                var hizmetToplami = ((TahakkukEditForm)OwnerForm).hizmetBilgileriTable.Tablo.DataController.ListSource
                    ...
                    .Where(x => x.HizmetId == hizmetId && !x.IptalEdildi && !x.Delete)
                    .Sum(x => x.BrutUcret);
                var indirimToplami = tablo.DataController.ListSource
                    .Cast<IndirimBilgileriL>()
                    .Where(x => x.HizmetId == hizmetId && !x.ManuelGirilenTutar && !x.IptalEdildi && !x.Delete)
                    .Sum(x => x.BrutIndirim);
        ...
        private (decimal brutIndirim, decimal kistDonemDusulenIndirim) IndirimHesapla(long indirimId, long hizmetId)
        {
            decimal HizmetToplamiAl(bool iptalEdildi)
            {
                var hizmetToplami = ((TahakkukEditForm)OwnerForm).hizmetBilgileriTable.Tablo.DataController.ListSource
                    .Cast<HizmetBilgileriL>()
                    .Where(x => x.HizmetId == hizmetId && x.IptalEdildi == iptalEdildi && !x.Delete)
                    .Sum(x => x.BrutUcret);
                var indirimToplami = tablo.DataController.ListSource
                    .Cast<IndirimBilgileriL>()
                    .Where(x => x.HizmetId == hizmetId && !x.ManuelGirilenTutar && x.IptalEdildi == iptalEdildi && !x.Delete)
                    .Sum(x => x.BrutIndirim);
            ...
            using (var bll = new IndiriminUygulanacagiHizmetBilgileriBll())
            {
                ...
                var indirimEntity = tablo.DataController.ListSource
                    .Cast<IndirimBilgileriL>()
        ...
        {
            if (tablo.DataRowCount == 0) return; // if (tablo.FocusedRowHandle < 0) return; filtre içeren tablolar için bunu kullan.
        ...
        protected internal void TopluHareketSil(long hizmetId)
        {
            ...
            var silinenKayifVarMi = false;

            source.ForEach(x => 
            {
                if (x.IptalEdildi || x.HizmetId != hizmetId) return;
                x.Delete = true;
                silinenKayifVarMi = true;
            });
            if (!silinenKayifVarMi) return;
            tablo.RefreshDataSource();
            ButtonEnabledDurumu(true);
        }
        ...
        protected override void IptalGeriAl()
        {
            ...
            {
                Messages.HataMesaji("İptal İşleminin Geri Alıması Durumunda Aynı İndirimden Birden Fazla Alım Durumu Oluşuyor.");
                return;
            }
            entity.IndirimAdi = entity.IndirimAdi.Remove(entity.IndirimAdi.Length - 27, 27); // TPx->1 (İptal edildi açıklaması 27 karakter olduğu için)
        ...
        protected internal void TopluIptalGeriAl(int hizmetHareketId)
        {
            var source = tablo.DataController.ListSource
                .Cast<IndirimBilgileriL>().
                Where(x => x.HizmetHareketId == hizmetHareketId);

            source.ForEach(x => 
            {
                x.IndirimAdi = x.IndirimAdi.Remove(x.IndirimAdi.Length - 27, 27); // TPx->1 (İptal edildi açıklaması 27 karakter olduğu için)
                x.IptalEdildi = false;
                x.IptalTarihi = null;
